...
class GroupFragment : Fragment(), CreateGroupListener {
    ...
    var contactList: List<Groups> = ArrayList<Groups>()
    ...
    override fun onCreateView(
        ...
        view.submit_btn.setOnClickListener {
            ...
            val value = ArrayList<Groups>()
            for (data in contactList) {
                if (data.isChecked){
                    value.add(data)
                }
            }
            viewModel.arrayData.value = value
    ...
    private fun initRecyclerView(quoteItem: List<Groups>) {

        val contactAdapter = GroupSelectAdapter(quoteItem, requireContext())
        recyclerview.apply {
